...
import { faClock , faCalendarAlt  , faStar } from '@fortawesome/free-solid-svg-icons'; //icons used in page
import { info } from "../data/info";

// date was given in number so below code converts them to month name
const dateString = '2024-02-26';
const dateParts = dateString.split('-');
const formattedDate = `${dateParts[2]}-${getMonthName(dateParts[1])}-${dateParts[0]}`;//adding month name

function getMonthName(month) {
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];//month name array
...
function Home() {
    const data = info.appointments; //data of user which we will map 
